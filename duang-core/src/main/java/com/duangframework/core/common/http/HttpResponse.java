...
public class HttpResponse extends HttpRequest {
    ...
    private static void pasrseHttpConn(HttpResponse response, HttpURLConnection httpConn,
        ...
        String type = response.getHeaderValue(HttpHeaders.CONTENT_TYPE);
        if (null != buff && null != type) {
            response.setEncoding(Const.ENCODING_FIELD);
            String[] split = type.split(";");
            response.setHttpContentType(FormatTypeEnums.mapAcceptToFormat(split[0].trim()));
