...
public class ParameterHelper {
    ...
    public static byte[] getFormData(Map<String, String> params) throws UnsupportedEncodingException {
        ...
        for (Map.Entry<String, String> entry : params.entrySet()) {
            ...
            result.append(URLEncoder.encode(entry.getKey(), Const.ENCODING_FIELD));
            result.append("=");
            result.append(URLEncoder.encode(entry.getValue(), Const.ENCODING_FIELD));
        }
        return result.toString().getBytes(Const.ENCODING_FIELD);
    }
}